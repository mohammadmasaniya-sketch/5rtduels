...
local State = {
        normalSpeed = 57, carrySpeed = 27, laggerSpeed = 13, laggerCarrySpeed = 13,
        speedType = "normal",
        laggerActive = false,
        laggerCarryActive = false,
        autoBatToggled = false,
        hittingCooldown = false, infJumpEnabled = true,
        antiRagdollEnabled = true, fpsBoostEnabled = true,
        guiVisible = true,
...
-- ========== WAYPOINTS (SZG-style: 4 per side, speed changes at waypoint 3) ==========
local leftWaypoints = {
...
local function getAutoMoveSpeed(phase)
        -- phase 1-2 = going speed, phase 3-4 = steal speed (SZG-style)
        if phase and phase >= 3 then
                ...
                else return State.carrySpeed end
        else
...
local function createMobilePanel()
        ...
        panel.Name = "DarkAngelDuelsButtons"
        ...
        -- CHANGE: moved buttons slightly higher (was 0.35, now 0.30)
        local BASE_Y = 0.30
        ...
        local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "DarkAngelBtn_dropbr.txt",
        ...
        local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "DarkAngelBtn_autoleft.txt",
                function(setActive)
        ...
        local autoBatSetActive = makeSeparateBtn("BtnAutoBat", "BAT\nAIMBOT",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "DarkAngelBtn_autobat.txt",
                function(setActive)
        ...
        local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "DarkAngelBtn_autoright.txt",
                function(setActive)
        ...
        local laggerCarrySetActive = makeSeparateBtn("BtnLaggerCarry", "LAGGER\nCARRY",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[4]), "DarkAngelBtn_laggercarry.txt",
                function(setActive)
                        toggleLaggerCarry()
        ...
        local laggerSetActive = makeSeparateBtn("BtnLaggerMode", "LAGGER\nMODE",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[4]), "DarkAngelBtn_laggermode.txt",
                function(setActive)
                        toggleLagger()
...
for _, name in pairs({"DarkAngelMiniGUI","DarkAngelMiniGUI"}) do
...
gui.Name="DarkAngelMiniGUI"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
gui.IgnoreGuiInset=true; gui.Parent=LP:WaitForChild("PlayerGui")

-- ========== TWINKLING STARS BACKGROUND ==========
local starsGui = Instance.new("ScreenGui")
starsGui.Name = "DarkAngelMiniStars"
starsGui.ResetOnSpawn = false
starsGui.DisplayOrder = 9
starsGui.IgnoreGuiInset = true
starsGui.Parent = LP:WaitForChild("PlayerGui")

local starsBg = Instance.new("Frame", starsGui)
starsBg.Size = UDim2.new(1, 0, 1, 0)
starsBg.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
starsBg.BackgroundTransparency = 0.85
starsBg.BorderSizePixel = 0
starsBg.ZIndex = 1
...
for i = 1, NUM_STARS do
        local star = Instance.new("Frame", starsBg)
        local sz = math_random(2, 5)
        star.Size = UDim2.new(0, sz, 0, sz)
        star.Position = UDim2.new(math_random() , 0, math_random(), 0)
        star.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
        star.BackgroundTransparency = math_random() * 0.4 + 0.1
        star.BorderSizePixel = 0
        star.ZIndex = 2
        Instance.new("UICorner", star).CornerRadius = UDim.new(1, 0)
        starObjects[i] = {frame = star, offset = math_random() * math.pi * 2, speed = 0.5 + math_random() * 1.5, baseAlpha = math_random() * 0.3 + 0.1}
end

local starTime = 0
RunService.RenderStepped:Connect(function(dt)
        starTime = starTime + dt
        for _, s in ipairs(starObjects) do
                local alpha = s.baseAlpha + math_sin(starTime * s.speed + s.offset) * 0.45
                s.frame.BackgroundTransparency = 1 - math.clamp(alpha, 0, 1)
        end
end)
-- ========== END STARS ==========
...
local function saveStealBarPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() writefile("DarkAngelStealBarPos.txt", str) end)
end

local function loadStealBarPosition()
    local savedPos = nil
    pcall(function() savedPos = readfile("DarkAngelStealBarPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
local function saveMainPosition()
        ...
        local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
        pcall(function() writefile("DarkAngelMiniGUIPos.txt", str) end)
end
...
local function saveMiniPosition()
        if miniBtn then
                ...
                local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
                pcall(function() writefile("DarkAngelMiniPos.txt", str) end)
        end
end

local function loadMainPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("DarkAngelMiniGUIPos.txt") end)
        if savedPos and savedPos ~= "" then
                local parts = {}
                for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
                if #parts >= 4 then
...
local function loadMiniPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("DarkAngelMiniPos.txt") end)
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="DARK ANGEL MINI"
...
miniBtn.Name = "DarkAngelMiniButton"
...
footerLbl.Text="dark angel mini  ·  v1.0"; footerLbl.TextColor3=Color3.fromRGB(50,50,50)
...
-- ========== SZG-STYLE 4-WAYPOINT PATROL ==========
-- Waypoints 1-2 use going speed, waypoints 3-4 use steal speed
...
startAutoLeft = function()
        ...
        Conns.autoLeft = RunService.Heartbeat:Connect(function()
                ...
                local phase = State.autoLeftPhase
                local target = leftWaypoints[phase]
                if not target then return end
                local spd = getAutoMoveSpeed(phase)
                local tgt = Vector3.new(target.X, root.Position.Y, target.Z)
                if (tgt - root.Position).Magnitude < 2.5 then
                        State.autoLeftPhase = phase + 1
                        if State.autoLeftPhase > #leftWaypoints then
                                -- Route complete
...
startAutoRight = function()
        ...
        Conns.autoRight = RunService.Heartbeat:Connect(function()
                ...
                local hum2 = char:FindFirstChildOfClass("Humanoid")
                if not root or not hum2 then return end
                local phase = State.autoRightPhase
                local target = rightWaypoints[phase]
                if not target then return end
                local spd = getAutoMoveSpeed(phase)
                local tgt = Vector3.new(target.X, root.Position.Y, target.Z)
                if (tgt - root.Position).Magnitude < 2.5 then
                        State.autoRightPhase = phase + 1
                        if State.autoRightPhase > #rightWaypoints then
                                -- Route complete
                                hum2:Move(Vector3.zero, false)
                                root.AssemblyLinearVelocity = Vector3.new(0, root.AssemblyLinearVelocity.Y, 0)
...
local function loadConfig()
        local hasFile=false; pcall(function() hasFile=isfile("DarkAngelMiniConfig.json") end)
        if not hasFile then return end
        local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("DarkAngelMiniConfig.json")) end)
